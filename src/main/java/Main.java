...
public class Main {
    ...
    public Main(ProductService productService, CartServiceImpl cartService) {
        this.productService = productService;
        this.cartService = cartService;
    }

    public static void main(String[] args) {
        new AnnotationConfigApplicationContext("java");
    }
            ...
            if (!str.isEmpty()) {
                ...
                if (command.equalsIgnoreCase("exit")) {
                    ...
                    if (command.equalsIgnoreCase("add")) {
                        // добавить продукт
                        Product product = productService.getProductById(Math.toIntExact(prodId));
                        if (product != null) {
                            cartService.addProduct(cart, productService.getProductById(Math.toIntExact(prodId)), quantity);
                            System.out.println("В корзину добавлен товар: " + productService.getProductById(Math.toIntExact(prodId)) + " - " + quantity + " шт.");
                        } else {
                            System.out.println("Такого товара нет в списке.");
                        }
                    } else if (command.equalsIgnoreCase("del")) {
                        // удалить продукт
                        if (cartService.getProductQuantity(cart, Math.toIntExact(prodId)) > 0) {
                            cartService.delProduct(cart, productService.getProductById(Math.toIntExact(prodId)), quantity);
                            System.out.println("Из корзины удален товар: " + productService.getProductById(Math.toIntExact(prodId)) + " - " + quantity + " шт.");
                        } else {
    ...
    private void cartTest() {
        ...
        productService.saveOrUpdate(new Product(8, "Product Added", BigDecimal.valueOf(888.00)));
        ...
        cartService.addProduct(cart, productService.getProductById(1), 1);
        cartService.addProduct(cart, productService.getProductById(2), 3);
        cartService.addProduct(cart, productService.getProductById(4), 9);
        cartService.addProduct(cart, productService.getProductById(5), 9);
        cartService.addProduct(cart, productService.getProductById(8), 5);
        // увеличить количество продуктов в корзине (добавить еще продукт с id=1 + 2шт.), итого 3шт.
        cartService.addProduct(cart, productService.getProductById(1), 2);
        // уменьшить количество одного продукта в корзине (id=2 - 1шт.), итого должно быть 2шт.
        cartService.delProduct(cart, productService.getProductById(2), 1);
        // удалить продукт из корзины
        cartService.delProduct(cart, productService.getProductById(4), 999);
        cartService.delProduct(cart, productService.getProductById(5), null);
